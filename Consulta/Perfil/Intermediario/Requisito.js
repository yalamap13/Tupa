...
$(document).ready(function() {
	tabla = $("#LlenarTabla").DataTable({
		"ajax": "Funcionalidad/Requisito/Listar.php",
            "order": [[ 1, "desc" ]],
            pageLength: 9,
            "columnDefs": 
              [
...
function Estado(id = null) {
      if(id) { 
                  $.ajax({
                        url: 'Funcionalidad/Requisito/Estado.php',
                        type: 'post',
...
function Actualizar(id = null) {
      if(id) {
            $("#TransferirId").remove();
            $.ajax({
                  url: 'Funcionalidad/Requisito/Llamar.php',
                  type: 'post',
                  data: {TransferirId : id},
                  dataType: 'json',
                  success:function(response) {
                        $("#TransferirCodigo").val(response.Codigo);
                        $("#TransferirDenominacion").val(response.Denominacion);
                        $(".AbrirModal").append('<input type="hidden" name="TransferirId" id="TransferirId" value="'+response.IdRequisito+'"/>');
                        $("#Formulario").unbind('submit').bind('submit', function() {
                                    var Aplicar = $(this);
                                    $.ajax({
                                    url: Aplicar.attr('action'),
