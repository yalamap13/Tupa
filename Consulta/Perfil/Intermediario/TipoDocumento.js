...
$(document).ready(function() {
	tabla = $("#LlenarTabla").DataTable({
		"ajax": "Funcionalidad/TipoDocumento/Listar.php",
        "order": [ 1, "desc" ],
        pageLength: 9,
        "columnDefs": 
              [
...
function Estado(id = null) {
      if(id) { 
                  $.ajax({
                        url: 'Funcionalidad/TipoDocumento/Estado.php',
                        type: 'post',
            ...
            $.ajax({success:function(response) {
                        ...
                        $("#Formulario").unbind('submit').bind('submit', function() {
                                    ...
                                    if(response.success == true) {
                                          ...
                                          tabla.ajax.reload(null, false);
                                          sweetAlert("Creación!", "Creo Correctamente...", "success");                                                           
                              } else {
...
function Actualizar(id = null) {
      if(id) {
            $("#TransferirId").remove();
            $.ajax({
                  url: 'Funcionalidad/TipoDocumento/Llamar.php',
                  type: 'post',
                  data: {TransferirId : id},
                  dataType: 'json',
                  success:function(response) {
                        $("#TransferirNombre").val(response.Nombre);
                        $(".AbrirModal").append('<input type="hidden" name="TransferirId" id="TransferirId" value="'+response.IdTipoDocumento+'"/>');
                        $("#Formulario").unbind('submit').bind('submit', function() {
                                    var Aplicar = $(this);
                                    $.ajax({
                                    url: Aplicar.attr('action'),
