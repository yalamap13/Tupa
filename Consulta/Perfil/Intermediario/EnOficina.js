...
$(document).ready(function() {
	...
	tabla = $("#LlenarTabla").DataTable({
		"ajax": "Funcionalidad/Expediente/ListarEnOficina.php",
        "order": [ 1, "desc" ],
        pageLength: 9,
        dom: 'fBtip',
        responsive: true,
...
function DerivarOficina(id = null) {
                $("#FormularioDerivar")[0].reset();
                $("#TransferirNroExpedienteDerivar").val(id);
                $('#FormularioDerivar').unbind('submit').bind('submit', function() {
                    ...
                    success: function(response) {
                        if (response == '0') {
                            $('#FormularioDerivar')[0].reset();
                            $('#AbrirModalDerivarOficina').modal('hide');
                            tabla.ajax.reload(null, false);
                            sweetAlert("Derivación a Oficina...!", "Expediente Derivado Correctamente...", "success");
                        } else {
                            $('#FormularioDerivar')[0].reset();
                            $('#AbrirModalDerivarOficina').modal('hide');
                            sweetAlert("Error al Crear Expediente o Datos Invalidos!", "Error", "warning");
...
function DerivarArchivo(id = null) {
                $("#FormularioArchivo")[0].reset();
                $("#TransferirNroExpedienteDerivarArchivo").val(id);
                $('#FormularioArchivo').unbind('submit').bind('submit', function() {
                    ...
                    success: function(response) {
                        if (response == '0') {
                            $('#FormularioArchivo')[0].reset();
                            $('#AbrirModalDerivarArchivo').modal('hide');
                            tabla.ajax.reload(null, false);
                            sweetAlert("Derivación a Archivo...!", "Expediente Derivado Correctamente...", "success");
                        } else {
                            $('#FormularioArchivo')[0].reset();
                            $('#AbrirModalDerivarArchivo').modal('hide');
                            sweetAlert("Error al Crear Expediente o Datos Invalidos!", "Error", "warning");
